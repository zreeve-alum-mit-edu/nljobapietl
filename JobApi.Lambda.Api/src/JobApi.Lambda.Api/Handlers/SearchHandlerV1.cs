...
public class SearchHandlerV1
{
    private readonly string _connectionString;
    private readonly string _openAiApiKey;
    private readonly AuditLogger _auditLogger;

    public SearchHandlerV1()
    {
    ...
    /// <summary>
    /// Searches for jobs using natural language and location filter (V1)
    /// </summary>
    public async Task<SearchResponse> Search(SearchRequest request, ILambdaContext context)
    {
        ...
        context.Logger.LogInformation($"[V1] Starting search: prompt='{request.Prompt}', numJobs={request.NumJobs}, location={request.City},{request.State}, miles={request.Miles}, onsite={request.IncludeOnsite}, hybrid={request.IncludeHybrid}, daysSince={request.DaysSincePosting}");
        ...
        try
        {
            ...
            context.Logger.LogInformation($"[V1] Search completed: found {jobs.Count} jobs");
