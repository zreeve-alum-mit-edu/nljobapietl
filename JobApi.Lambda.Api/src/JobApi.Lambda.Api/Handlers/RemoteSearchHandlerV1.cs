...
public class RemoteSearchHandlerV1
{
    ...
    /// <summary>
    /// Searches for remote jobs using natural language and date filter (V1)
    /// </summary>
    public async Task<SearchResponse> SearchRemote(RemoteSearchRequest request, ILambdaContext context)
    {
        ...
        context.Logger.LogInformation($"[V1] Starting remote search: prompt='{request.Prompt}', numJobs={request.NumJobs}, daysSince={request.DaysSincePosting}");
        ...
        try
        {
            ...
            context.Logger.LogInformation($"[V1] Remote search completed: found {jobs.Count} jobs");
