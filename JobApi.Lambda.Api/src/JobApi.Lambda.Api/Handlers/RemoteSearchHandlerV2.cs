...
public class RemoteSearchHandlerV2
{
    ...
    /// <summary>
    /// Searches for remote jobs using natural language and date filter (V2)
    /// </summary>
    public async Task<SearchResponse> SearchRemote(RemoteSearchRequest request, ILambdaContext context)
    {
        ...
        context.Logger.LogInformation($"[V2] Starting remote search: prompt='{request.Prompt}', numJobs={request.NumJobs}, daysSince={request.DaysSincePosting}");
