...
namespace JobApi.Lambda.LlmBatchCheck;
...
public class Function
{
    ...
    public Function(IAmazonS3 s3Client, HttpClient httpClient, string bucketName)
    {
        _s3Client = s3Client;
    ...
    /// <summary>
    /// Lambda handler for checking LLM batch status
    /// Triggered hourly by EventBridge
    /// </summary>
    public async Task FunctionHandler(ILambdaContext context)
    {
        context.Logger.LogInformation("=== LLM Batch Check Started ===");
        ...
        context.Logger.LogInformation("=== LLM Batch Check Complete ===");
    }
    ...
    private async Task DownloadAndUploadResults(WorkplaceBatch batch, string outputFileId, ILambdaContext context)
    {
        ...
        // Upload to S3
        var s3Key = $"llmbatch/batchresults/{filename}";
    ...
    private async Task DownloadAndUploadErrorFile(WorkplaceBatch batch, string errorFileId, ILambdaContext context)
    {
        // Get file info to get the original filename
