...
public class LocationBatchCheckStage
{
    private readonly HttpClient _httpClient;
    private readonly string _apiKey;
    private readonly string _locationResultFolder;

    public LocationBatchCheckStage(string dataRootPath)
    {
    ...
    {
        Console.WriteLine("=== LOCATION BATCH CHECK STAGE ===");
    ...
    private async Task DownloadResults(LocationBatch batch, string outputFileId)
    {
        ...
        // Save to locationresult folder with a unique name
        var fileName = $"location_results_{batch.FileId}_{Path.GetFileName(batch.BatchFilePath)}";
        var filePath = Path.Combine(_locationResultFolder, fileName);
    ...
    private async Task DownloadErrorFile(LocationBatch batch, string errorFileId)
    {
        ...
        // Save to locationresult folder with a unique name indicating it's an error file
        var fileName = $"location_errors_{batch.FileId}_{Path.GetFileName(batch.BatchFilePath)}";
        var filePath = Path.Combine(_locationResultFolder, fileName);
