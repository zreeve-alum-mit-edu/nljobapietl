...
public class JobImporter
{
    public static async Task<int> Main(string[] args)
    {
        if (args.Length == 0)
        {
            Console.WriteLine("Usage: dotnet run --project JobApi.csproj -- <path-to-jsonl-file>");
            return 1;
        }
    ...
    private static async Task ImportJobs(string filePath)
    {
        ...
        while (await reader.ReadLineAsync() is { } line)
        {
            ...
            try
            {
                ...
                if (jobs.Count >= batchSize)
                {
                    ...
                    totalInserted += jobs.Count;
                    Console.WriteLine($"Inserted {totalInserted} jobs (line {lineCount})...");
                    jobs.Clear();
    ...
    private static Job MapToJob(JsonlJob jsonJob, Guid fileId)
    {
        return new Job
        {
            ...
            WorkplaceType = Truncate(jsonJob.Json?.SchemaOrg?.JobLocationType ?? jsonJob.Json?.JsonLD?.JobLocationType, 50),
