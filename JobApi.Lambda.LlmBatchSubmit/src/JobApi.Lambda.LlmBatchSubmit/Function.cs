...
namespace JobApi.Lambda.LlmBatchSubmit;
...
public class Function
{
    ...
    /// <summary>
    /// Lambda handler for submitting LLM batch files to OpenAI
    /// Triggered when a file is uploaded to the llmbatch/createdbatches/ folder
    /// </summary>
    public async Task FunctionHandler(S3Event s3Event, ILambdaContext context)
    {
        foreach (var record in s3Event.Records)
        {
            ...
            catch (Exception ex)
            {
                ...
                try
                {
                    await MoveBatchFile(bucketName, key, "llmbatch/batchuploadfailure", context);
                }
    ...
    private async Task ProcessBatchFile(string bucketName, string key, ILambdaContext context)
    {
        ...
        context.Logger.LogInformation("Creating workplace_batches tracking record...");
        await CreateWorkplaceBatchRecord(key, openAiInputFileId, openAiBatchId, context);

        // Move file to success folder
        context.Logger.LogInformation("Moving file to batchsubmitted folder...");
        await MoveBatchFile(bucketName, key, "llmbatch/batchsubmitted", context);
    ...
    private async Task CreateWorkplaceBatchRecord(string s3Key, string openAiInputFileId, string openAiBatchId, ILambdaContext context)
    {
        ...
        db.WorkplaceBatches.Add(workplaceBatch);
        await db.SaveChangesAsync();

        context.Logger.LogInformation($"Created workplace_batches record: {workplaceBatch.Id}");
    }
