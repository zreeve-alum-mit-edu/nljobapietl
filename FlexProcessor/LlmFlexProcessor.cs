// Standalone LLM Flex Processor - processes batch files using OpenAI's flex service tier with dynamic rate limiting
// Compile and run: dotnet-script LlmFlexProcessor.cs
// Or: csc LlmFlexProcessor.cs && mono LlmFlexProcessor.exe
...
class LlmFlexProcessor
{
    // Dynamic rate limiting settings
    private static int _currentMaxConcurrency = 100;
    ...
    private const string BatchFolder = "/mnt/c/GIT/JobApi.New/Data/llmbatch";
    private const string ResultFolder = "/mnt/c/GIT/JobApi.New/Data/llmresult";
    private const string ErrorFolder = "/mnt/c/GIT/JobApi.New/Data/llmbatcherrored";
    ...
    {
        Console.WriteLine("=== LLM Flex Processor (Dynamic Rate Limiting) ===");
        ...
        // Get all batch files
        var batchFiles = Directory.GetFiles(BatchFolder, "workplace_batch_*.jsonl")
            .OrderBy(f => f)
            .ToList();
    ...
    {
        // Extract GUID from workplace_batch_<guid>_<number>.jsonl
        var parts = fileName.Replace("workplace_batch_", "").Replace(".jsonl", "").Split('_');
    ...
    private static async Task<bool> ProcessBatchFile(string batchFilePath, string batchId, int currentBatch, int totalBatches)
    {
        try
        {
            ...
            var resultFilePath = Path.Combine(ResultFolder, $"workplace_results_{batchId}.jsonl");
            var errorFilePath = Path.Combine(ErrorFolder, $"workplace_errors_{batchId}.jsonl");
        ...
        for (int i = 0; i < maxRetries; i++)
        {
            try
            {
                ...
                var sql = errorMessage == null
                    ? "UPDATE workplace_batches SET status = @status WHERE id = @id"
                    : "UPDATE workplace_batches SET status = @status, error_message = @error WHERE id = @id";
                ...
                if (rowsAffected == 0)
                {
                    Console.WriteLine($"Note: No workplace_batches record found for batch {batchId} (ignoring)");
                }
